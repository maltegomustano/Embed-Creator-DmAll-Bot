...
async def dm_all(server_id):
    try:
        ...
        if guild:
            message_content = input((Colorate.Color(Colors.blue, "Enter the message to send to all members or type 'config' to use config.js>
            ...
            if "config" in message_content.lower():
                ...
                cooldown_embed = await asyncio.to_thread(lambda: float(input((Colorate.Color(Colors.blue, "Enter the cooldown duration for e>
                ...
                for member in guild.members:
                    if not member.bot:
                        try:
                            ...
                            print((Colorate.Color(Colors.green, f"[+] Embed Sent to {member.name} ({member.id}) - Time taken: {end_time_memb>
                ...
                cooldown_message = await asyncio.to_thread(lambda: float(input((Colorate.Color(Colors.blue, "Enter the cooldown duration for>
                ...
                for member in guild.members:
                    if not member.bot:
                        try:
                            ...
                            end_time_member = time.time()
                            print((Colorate.Color(Colors.green, f"[+] Message Sent to {member.name} ({member.id}) - Time taken: {end_time_me>
                            members_sent += 1
                ...
                print((Colorate.Color(Colors.blue, f"[!] Command Used: DM All - {members_sent} messages sent, {members_fail} messages failed>
        else:
...
bot_token = input((Colorate.Color(Colors.blue, "Enter Bot Token:MTMyMzkxMTY0ODAyODc4NjczOQ.GeWcYw.n3Sp67-ZWOp8wpU3-iG-8qeJg22zSHYBGxiyKA ")))
server_id = input((Colorate.Color(Colors.blue, "Enter Server ID:1318272823948738602 ")))
